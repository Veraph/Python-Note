...
解决CPU高速执行能力和IO设备龟速不匹配问题的另一种解决办法  
不同于同步IO，异步IO主线程在“发出IO请求”到收到“IO完成”这段时间里不会挂起，而是在消息循环中继续处理其他信息。  
##### 协程(微线程)(Coroutine)
1. 一个线程执行，最大的优势是执行效率极高，因为没有线程切换的开销；第二大优势是不需要多线程的锁机制。  
2. 使用多进程+协程可以拥有极高的性能。  
3. Python中对协程的支持是通过generator实现的。  
4. 子程序(函数)就是协程的一种特例
##### asyncio
标准库，直接内置了对异步IO的支持,可以实现单线程并发IO操作  
如果把asyncio用在服务器端(如web服务器)，由于HTTP连接是IO操作，所以可以实现多用户的高并发支持  
